...
A timeline of the process every software change goes through at Pitang, rendered on a single rail:

**Software changed → Compound Engineering → Plan/Work → Review → Compound → Compound Design → Pitang Knowledge ↺**

The last step loops back into the first — everything learned becomes Pitang Knowledge and the next change starts from a higher baseline.

The steps and their descriptions live in [`lib/lifeline-processo.ts`](lib/lifeline-processo.ts); the page is [`app/page.tsx`](app/page.tsx). Built on the [Lifeline](https://github.com/evilrabbit/lifeline) boilerplate by Evil Rabbit — the component docs follow below.
